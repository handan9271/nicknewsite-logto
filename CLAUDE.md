...
### Stage 4: 学习曲线 + 阶段成就 + VIP 体系 ✅（2026-04-11 完成）

**核心理念：** 学生在网站上的学习是有时间轴和阶段性的，每提升 0.5 分解锁一个里程碑。VIP 学员享有 AI 个性化学习计划等专属权益。

**数据库新增：**
- `users` 加 `is_vip`, `starting_band`, `current_band`, `target_band`, `exam_date`
- `learning_reports` — 保存生成的学习报告
- `milestones` — 阶段成就（每 0.5 分一个）
- `study_plans` — VIP 学习计划

**教师功能：**
- [x] 学情设置（起始/当前/目标分数 + 考试日期）
- [x] 当前 band 提升 0.5 分自动生成里程碑
- [x] 学习报告自动保存，可查看历史/编辑/复制
- [x] AI 学习计划生成（VIP 专属，基于目标分数+考试日期）
- [x] 学生详情含完整时间轴、报告列表、计划列表
- [x] 进度条显示学习进展百分比

**学生功能（主站）：**
- [x] 侧边栏新增"学习时间轴"入口
- [x] 个人学习档案卡片（起始/当前/目标 Band + 进度条）
- [x] 阶段成就时间轴
- [x] 学习报告查看（折叠展开）
- [x] 学习计划查看（VIP 专属，非 VIP 显示升级提示）
- [x] 侧边栏 VIP 徽章

**Admin 功能：**
- [x] 用户编辑模态加 VIP 切换
- [x] 用户列表显示 VIP 徽章
- [x] 用户列表 API 返回 is_vip / current_band / target_band
...
### Stage 5: 口语模考深度打磨 ✅（2026-04-13 至 2026-04-15）

**核心理念：** Stage 1-4 把平台骨架搭完，Stage 5 把口语模考这个核心产品打磨到可用。两天半内 25+ commit，围绕真题、检测、评分、报告、UX 全面迭代。

**真题题库：**
- [x] 574 道真题题库接入（`4d3b884`）
- [x] 按主题出题 + 练习页题库浏览器
- [x] Part 检测全面修复（正则优化）+ 三 Part 写作风格区分

**AI 评分 V2 校准（`188a1cc`）：**
- [x] 36 个真实样本作为校准集
- [x] Few-shot prompt + 严格 GRA（Grammatical Range & Accuracy）标准
- [x] Train/test 分离验证，避免过拟合

**PDF 模考报告（`823983f`）：**
- [x] 后端生成，AI 深度分析
- [x] 中文字体支持 + Unicode 特殊字符处理（em dash 等）
- [x] Verdict 页面内直接显示，不再强制下载
- [x] 报告自动归档到历史记录

**目标分数体系（`87aaea1`）：**
- [x] Band 5/6/7/8/9 目标分数选择
- [x] 基础版最低 Band 4（Band 5 目标时）
- [x] 学生水平高于目标分数时的兜底逻辑
- [x] 学习报告融合模考详细分析数据

**UX 打磨：**
- [x] 倒计时秒数移到输入框进度条旁（不再挤在右上角 HUD）
- [x] 模考答题加暂停/继续按钮（单人显示，多人隐藏以保公平）
- [x] Part 2 cue card 智能提示点

---

## 避坑记录

### 🚨 Logto session 全局串号（`6cc98c3`，2026-04-11）
**症状：** 所有用户共用同一个 Logto session，A 用户登录后 B 访问会直接变成 A。
**根因：** `DatabaseSessionStorage` 被实现为模块级单例/全局共享，所有请求读写同一份 session 数据。
**教训：** 任何 session storage 必须按 session_id/user_id 严格隔离。后续任何涉及用户态缓存/存储的改动都要复核隔离边界。
**相关反例：** `43aac43` 也是同类问题（Logto 用户缓存导致登录不一致，后来回滚）。
